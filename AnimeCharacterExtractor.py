...
JPEG_QUALITY = int(input('请输入输出图像的JPEG质量（0-100，回车使用默认参数：100）：') or 100)
DETECTION_MODE = int(input('请输入人脸检测模式（0：直接在每一帧上进行人脸检测；1：计算余弦相似度，跳过相似的帧；2：使用图像哈希比较，跳过相似的帧。回车使用默认参数：0）：') or 0)
USE_FFMPEG = int(input('是否使用ffmpeg提取关键帧进行人脸检测（0：否，1：是，默认：0）：') or 0)
...
if USE_FFMPEG == 1:
    # 创建临时文件夹
    TEMP_FOLDER = "temp_keyframes"
    if not os.path.exists(TEMP_FOLDER):
        os.makedirs(TEMP_FOLDER)

    # 使用ffmpeg提取关键帧，保存为jpg格式，文件名以关键帧的时间戳命名
    command = f'ffmpeg -i "{VIDEO_PATH}" -vf "select=eq(pict_type\\,I)" -vsync vfr "{TEMP_FOLDER}/keyframe_%03d.jpg"'
